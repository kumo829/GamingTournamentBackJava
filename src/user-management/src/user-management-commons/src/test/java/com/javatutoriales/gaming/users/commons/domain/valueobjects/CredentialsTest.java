...
class CredentialsTest {
    ...
    class BuilderTest {
        ...
        void givenAnCredentialsBuilderWithNotAllRequiredAttributesProvided_whenInvokingTheBuildMethod_thenAnExceptionIsThrown(Credentials.CredentialsBuilder credentialsBuilder, String errorField, String errorMessage) {
            var ex = Assertions.assertThrows(ConstraintViolationException.class, credentialsBuilder::build);
            assertThat(ex.getMessage()).contains(errorField).contains(errorMessage);
        }

        static Stream<Arguments> credentialBuilderErrors() {
            return Stream.of(
                    Arguments.arguments(Credentials.builder().username("username"), "password", "must not be empty"),
                    Arguments.arguments(Credentials.builder().username(""), "password", "must not be empty"),
                    Arguments.arguments(Credentials.builder().password(""), "username", "must not be empty"),
                    Arguments.arguments(Credentials.builder().password("password"), "username", "must not be empty"),
                    Arguments.arguments(Credentials.builder().username("username").password("abc"), "password", "size must be between 8 and 60")
            );
        }
