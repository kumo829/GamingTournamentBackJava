...
class KafkaStorageOutputPortTest {
    ...
    void givenAnAccountRegisteredEvent_whenTheAccountIsRegistered_thenAMessageIsSentToTheAccountRegisteredEventTopic() throws Exception {
        ...
        assertThat(message).has(key(accountId))
                .has(value(jsonEvent))
                .has(partition(0));

        JsonPath jsonPath = new JsonPath(message.value());

        assertThat(jsonPath.getUUID("account.id")).isEqualTo(accountId);
        assertThat(jsonPath.getString("account.firstName")).isEqualTo(firstName);
        assertThat(jsonPath.getString("account.lastName")).isEqualTo(lastName);
        assertThat(jsonPath.getString("account.email")).isEqualTo(email);
        assertThat(jsonPath.getString("account.username")).isEqualTo(username);
        assertThat(jsonPath.getString("account.password")).isEqualTo(password);
        assertThat(jsonPath.getString("account.profile")).isEqualTo(Profile.ADMIN.toString());
    }
